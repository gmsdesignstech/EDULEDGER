...
import { getInstitution, getReceipt, type DbPayment } from "@/lib/db";
...
export async function GET(_:Request,{params}:{params:Promise<{receipt:string}>}) {
  ...
  const number=decodeURIComponent((await params).receipt),receipt=await getReceipt(user.institutionId,number);
  if(!receipt)return Response.json({error:"Receipt not found"},{status:404});
  const school=await getInstitution(user.institutionId),doc=await PDFDocument.create(),page=doc.addPage([612,792]),font=await doc.embedFont(StandardFonts.Helvetica),bold=await doc.embedFont(StandardFonts.HelveticaBold);
  const [logoBytes,signatureBytes]=await Promise.all([readFile(join(process.cwd(),"public","eduledger-logo.jpeg")),readFile(join(process.cwd(),"public","authorized-signature.jpg"))]),logo=await doc.embedJpg(logoBytes),signature=await doc.embedJpg(signatureBytes);
...
type ImageRef=Awaited<ReturnType<PDFDocument["embedJpg"]>>;
...
function drawCopy(page:PDFPage,top:number,copy:string,receipt:DbPayment,school:School,font:PDFFont,bold:PDFFont,logo:ImageRef,signature:ImageRef) {
  ...
  const contact=[school.address,school.phone&&`Phone: ${school.phone}`,school.email].filter(Boolean).join(" | ")||"School Administration";
  ...
  centered("FEE PAYMENT RECEIPT",top-92,12,true,blue);
  ...
  text(`Admission No: ${receipt.admission}${receipt.rollNumber?`  |  Roll No: ${receipt.rollNumber}`:""}`,left,top-142,7.5,true);
  ...
  text(`[ ${copy} ]`,left,top-153,6.5,true,muted);
  ...
  text(`Mode: ${receipt.method}`,left,tableTop-60,7.2,true);
  const transaction=`Transaction ID: ${receipt.transactionId||"Not provided"}`;
  text(fit(transaction,font,7.2,220),right-font.widthOfTextAtSize(fit(transaction,font,7.2,220),7.2),tableTop-60,7.2);
  text(`Amount Paid: ${words}`,left,tableTop-76,7.4,true);
  page.drawLine({start:{x:left,y:tableTop-84},end:{x:right,y:tableTop-84},thickness:.6,color:line,dashArray:[2,2]});
  const signatureY=tableTop-116;
  ...
  page.drawLine({start:{x:455,y:signatureY},end:{x:right,y:signatureY},thickness:.8,color:ink});centeredAt(text,bold,"Authorized Signatory",506.5,signatureY-11,7,true);
}
