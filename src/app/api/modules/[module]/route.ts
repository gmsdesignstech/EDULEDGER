...
import { z } from "zod";
import { addModuleRecord, listModuleRecords } from "@/lib/db";
...
export async function POST(
  ...
  if (!config)
    return NextResponse.json({ error: "Unknown module" }, { status: 404 });
  if (
    !["SCHOOL_ADMIN", "SUPER_ADMIN", "TEACHER", "ACCOUNTANT", "STAFF"].includes(
      user.role,
    )
  )
  ...
  if (Object.keys(clean).length !== config.fields.length)
    return NextResponse.json(
      { error: "Complete every required field." },
      { status: 400 },
    );
  return NextResponse.json(
    { record: await addModuleRecord(user.institutionId, module, clean) },
    { status: 201 },
  );
}
