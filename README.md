...
EduLedger is a production-oriented, multi-role school operations platform built with Next.js, TypeScript, Tailwind CSS, Prisma and PostgreSQL.
...
Local authentication and student management use the built-in SQLite backend at `data/eduledger.db`. The database is created automatically, passwords are bcrypt-hashed, and sessions use random server-side tokens in HTTP-only cookies.

For the separate PostgreSQL production schema, copy `.env.example` to `.env`, configure `DATABASE_URL`, run `npm run db:generate`, and apply a migration before switching the data adapter.
...
The authenticated dashboard now uses the local SQLite ledger as a single source of truth for students, teachers, classes, attendance, fee invoices, payments, receipts, activity, notifications, settings and audit history. Existing local records are upgraded in place at startup; the database is not reset.
...
Razorpay configuration uses `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`, and `RAZORPAY_WEBHOOK_SECRET`. Configure the webhook URL as `https://YOUR_DOMAIN/api/subscription/webhook` and subscribe to `payment.captured`, `payment.failed`, and `order.paid`. Local SQLite tables are upgraded and seeded automatically without resetting existing records. PostgreSQL deployments should run `npx prisma migrate deploy`.

## Production setup

- Create a managed PostgreSQL database and run `prisma migrate deploy` during release.
- Add every variable from `.env.example` in Vercel. Never expose provider secrets as `NEXT_PUBLIC_*` values.
...
The Prisma model is tenant-aware through `institutionId`, includes indexed operational tables, payment/audit entities, and explicit role types. Add row-scoping to every query (`institutionId` from the trusted session), use database transactions for admissions and payments, and enqueue slow work such as PDFs, email and imports. Health, sitemap, robots, security headers and reduced-motion behavior are included.
